...
router.post('/request', async (req, res) => {
  try {
    ...
    const existing = await db.collection('chatRequests')
...
router.post('/respond', async (req, res) => {
  try {
    ...
    if (data.toUid !== uid) return res.status(403).json({ error: 'Not your request.' });
    if (data.status !== 'pending') return res.status(400).json({ error: 'Already handled.' });
