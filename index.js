...
io.on("connection", (socket) => {
  ...
  socket.on("join-day", (date) => socket.join(date));
  socket.on("join-answer", (answerId) => socket.join(`answer:${answerId}`));
  socket.on("leave-answer", (answerId) => socket.leave(`answer:${answerId}`));
...
cron.schedule(CRON_SCHEDULE, async () => {
  try {
    ...
    console.log(`[CRON] Winner computed →`, res.data?.winner?.displayName || "No winner");

    const qRef = db.collection("questions").doc(today);
    await qRef.set({ closedAt: admin.firestore.FieldValue.serverTimestamp(), isActive: false }, { merge: true });

    const nextRef = db.collection("questions").doc(tomorrow);
    const nextDoc = await nextRef.get();
    if (nextDoc.exists) {
      await nextRef.set({ openedAt: admin.firestore.FieldValue.serverTimestamp(), isActive: true }, { merge: true });
